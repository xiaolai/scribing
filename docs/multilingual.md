...
Stroke order and script support are separate claims. The font catalogue covers 120 script entries for rendering. Ordered motor data exists for four groups: chinese 9,574, japanese 6,636, english 52 and korean 40. The Korean units are isolated jamo, and the preparer composes them. It decomposes a syllable through the Unicode arithmetic, splits compound finals, and fits each jamo to its own region of the ink, which reaches the 7,448 syllables whose vowel is not a wrapping one. Wrapping vowels ㅘㅙㅚㅝㅞㅟㅢ are declined and fall back to a generated drawing sequence, because a rectangular region cannot separate one from the initial it surrounds; that is a known gap, recorded with its cause in [the survey](../dev-docs/research/20260911-normative-stroke-order.md). Devanagari is deferred: Wikimedia Commons holds 85 stroke-order files for it, of which only 13 are vector, so importing it resembles authoring rather than importing.
