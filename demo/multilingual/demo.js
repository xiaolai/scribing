...
(() => {
  ...
  const busy = (value) => {
    ...
    actions.forEach((id) => { $(id).disabled = value; });
    $('unit').disabled = value;
    $('plan').disabled = value;
    $('renderer').disabled = value;
    $('tolerance').disabled = value;
  };
  const stop = () => { action++; if (writer) writer.cancelQuiz(); };
  const size = () => Math.max(64, $('writing-area').clientWidth);
  const createWriter = () => {
    if (writer) writer.destroy();
    writer = new Scribing('writing-area', {
      width: size(), height: size(), padding: 24, renderer: $('renderer').value,
      showCharacter: true, showOutline: true, strokeColor: '#242424',
      highlightColor: '#246bb0', outlineColor: '#dddddd', drawingColor: '#246bb0',
    });
  };
  const loadJSON = async (path, signal) => {
    const response = await fetch(path, { signal });
    if (!response.ok) throw new Error(`Cannot load ${path} (${response.status}). Build the data packs and serve the repository over HTTP.`);
  ...
  const loadUnit = async (id) => {
    ...
    try {
      ...
      options($('plan'), unit.plans.map((plan) => [plan.id, plan.id]));
  ...
  const loadPack = async () => {
    ...
    try {
      ...
      options($('unit'), Object.entries(pack.units).map(([id, unit]) => [id, unit.text === id ? id : `${unit.text} — ${id}`]));
      createWriter();
  ...
  $('plan').addEventListener('change', () => { stop(); status('Order plan selected for animation and new practice sessions.'); });
  $('tolerance').addEventListener('change', () => { stop(); status('Practice stopped. Matching tolerance changed; start a new session to use it.'); });
  $('lookup').addEventListener('submit', (event) => { event.preventDefault(); if (provider) loadUnit($('text').value.trim()); });
  $('retry').addEventListener('click', () => catalog ? loadPack() : initialize());
  $('animate').addEventListener('click', () => run(async () => { status('Animating selected stroke plan…'); await writer.animateCharacter({ planId: $('plan').value }); }));
  const quiz = (guided) => run(async () => {
    const token = action;
    const tolerance = $('tolerance');
    const label = tolerance.options[tolerance.selectedIndex].textContent;
    status(`${guided ? 'Trace the model, starting with the highlighted stroke.' : 'Write the selected unit from memory.'} ${label} matching against this source model.`);
    await writer.quizUnit({
      guided, leniency: Number(tolerance.value), planId: $('plan').value, acceptAlternatePlans: $('alternatives').checked,
      onCorrectStroke: (event) => { if (token === action) status(`Accepted. ${event.strokesRemaining} stroke(s) remaining.`); },
      onMistake: (event) => { if (token === action) status(`Try again: ${event.reason.replace(/-/g, ' ')}.`); },
      onComplete: (event) => { if (token === action) status(`Complete · ${event.totalMistakes} mistake(s). This checks the selected source model.`); },
    });
  });
  $('guided').addEventListener('click', () => quiz(true));
  $('practice').addEventListener('click', () => quiz(false));
  $('show').addEventListener('click', () => run(async (token) => { await writer.showCharacter(); if (token === action) status('Model shown.'); }));
  $('stop').addEventListener('click', () => run(async (token) => { await writer.showCharacter(); if (token === action) status('Practice stopped.'); }));
  window.addEventListener('resize', () => { if (writer) writer.updateDimensions({ width: size(), height: size() }); });
  window.addEventListener('pagehide', () => { generation++; stop(); if (pending) pending.abort(); if (writer) writer.destroy(); });
  initialize();
})();
