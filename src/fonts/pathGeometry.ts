...
function evaluateBezier(points: Point[], t: number): Point {
  let current = points.map((v) => v.slice() as Point);
  while (current.length > 1) {
    current = current
      ...
      .map(
        (v, i) =>
          [
            v[0] * (1 - t) + current[i + 1][0] * t,
            v[1] * (1 - t) + current[i + 1][1] * t,
          ] as Point,
      );
  }
  return current[0];
 ...
 */
function extremumParameters(axisValues: number[]): number[] {
...
function createBoundsAccumulator() {
  ...
  return {
    ...
    finish() {
      ...
      return { minX, minY, maxX, maxY, area, contours };
    },
  };
}

/** Font-outline SVG subset (lines and quadratic/cubic curves), with true curve extrema. */
export default function pathGeometry(path: string) {
  ...
  const reflected = (kinds: string[]): Point =>
    kinds.indexOf(previous) >= 0
      ? [2 * cursor[0] - lastControl[0], 2 * cursor[1] - lastControl[1]]
      : cursor;

  const step = (type: string, values: number[], i: number, relative: boolean) => {
    const point = (offset: number): Point => [
      values[i + offset] + (relative ? cursor[0] : 0),
      values[i + offset + 1] + (relative ? cursor[1] : 0),
    ];
  ...
  for (const { command, values } of groups) {
    ...
    if (
      count === undefined ||
      (count === 0 ? values.length !== 0 : !values.length || values.length % count !== 0)
    ) {
      fail();
    }
    if (upper === 'Z') {
