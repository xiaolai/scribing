...
async function main() {
  ...
  assert(rule('#target').includes('width: calc(100% - 32px)'), 'Writing area must fit narrow viewports');
  assert(rule('#target').includes('aspect-ratio: 1'), 'Writing area must retain square proportions');
  assert(rule('#target svg').includes('width: 100%'), 'SVG must scale with its target');
  assert(rule('.actions').includes('auto-fit'), 'Action buttons must wrap in narrow viewports');

  const dom = new JSDOM(html, { url: 'https://example.test/demo/#%E0%A4%A', runScripts: 'outside-only' });
  const { window } = dom;
  const loads = [];
  let instances = 0;
  class Writer {
    constructor() {
      instances++; this.shown = false;
      window.document.querySelector('#target').appendChild(window.document.createElementNS('http://www.w3.org/2000/svg', 'svg'));
    }
    setCharacter(character) {
      return new Promise((resolve, reject) => loads.push({ character, resolve, reject }));
    }
    hideCharacter() { this.shown = false; return Promise.resolve(); }
    showCharacter() { this.shown = true; return Promise.resolve(); }
    showOutline() { return Promise.resolve(); }
    hideOutline() { return Promise.resolve(); }
    animateCharacter() { return Promise.resolve(); }
    quiz() { return Promise.resolve(); }
  }
  ...
  assert.strictEqual(loads[0].character, '我', 'Malformed fragment must use default character');
  ...
  submit('𠮷');
  assert.strictEqual(loads[1].character, '𠮷', 'Supplementary characters must remain intact');
  submit('你');
  loads[1].resolve(); await flush();
  assert($('.js-animate').disabled, 'Stale completion must not enable controls');
  loads[2].reject(new Error('offline')); await flush();
  ...
  submit('好'); loads[3].resolve(); await flush();
  assert(!$('.js-quiz').disabled, 'Retry must restore actions');
  assert.strictEqual(instances, 1, 'Character updates must reuse the writer and listeners');
  submit('你好');
  ...
  const realDom = new JSDOM(html, { url: 'https://example.test/demo/', runScripts: 'outside-only' });
  ...
  assert.strictEqual(svg.getAttribute('viewBox'), '0 0 400 400', 'The real SVG needs a logical viewBox to scale without clipping');
  ...
  assert.strictEqual(realToggle.getAttribute('aria-pressed'), 'true', 'Animate must immediately synchronize character visibility');
  realToggle.click();
  assert.strictEqual(realToggle.getAttribute('aria-pressed'), 'false', 'Toggle after Animate must hide the character');
  realWindow.writer.destroy();
  realDom.window.close();
  console.log('Demo smoke passed: Unicode input, loading/retry, stale loads, writer reuse, and real-bundle SVG scaling/visibility.');
}
