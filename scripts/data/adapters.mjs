...
function flattenCubic(a, b, c, d, tolerance, out, depth = 0) {
  const excess =
    distance(a, b) + distance(b, c) + distance(c, d) - distance(a, d);
...
export function parseOmniglot(text) {
  ...
  for (const line of lines) {
    if (line === "BREAK") {
      ...
      if (parts.length !== 3 || parts.some((x) => !x.trim()))
        fail("Invalid XYT row");
...
export function parseGlyphLiteral(text) {
  if (typeof text !== "string" || text.length > 100000)
    fail("Glyph literal limit");
  // A full-file whitelist, never eval/import the downloaded TypeScript.
  const clean = text
    .replace(/\/\*[\s\S]*?\*\//g, "")
    .replace(/^import type[^;]*;/gm, "")
    .trim();
  const match = /^export const \w+(?:\s*:\s*\w+)?\s*=\s*\{\s*width:\s*([\d.]+),\s*variants:\s*\[([\s\S]*?)\],?\s*\};?$/.exec(
    clean
  );
  ...
  if (!finite(width) || width <= 0 || !strings.length)
    fail("Invalid glyph metrics");
...
export function letterpathsUnit(data) {
  ...
  for (const s of data.strokes) {
    if (s.kind === "mark") {
      ...
      for (const c of curves) {
        const p = ["p0", "p1", "p2", "p3"].map((k) =>
          point([c[k]?.x, c[k]?.y])
        );
        if (end && distance(end, p[0]) > 0.001)
          fail("Discontinuous motor stroke");
        path += `${end ? "" : "M" + p[0].join(" ")} C${p
          .slice(1)
          .flat()
          .join(" ")}`;
        end = p[3];
  ...
  const unit = makeUnit(
    ...
    strokes,
    coordinates,
    { width: 20, dots, script: "Latn", style: "letterpaths-print" }
  );
