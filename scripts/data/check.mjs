...
const validateUnit = require(path.join(runtimeRoot, 'src/units/validateUnit.ts')).default;
const compileUnit = require(path.join(runtimeRoot, 'src/units/compileUnit.ts')).default;
...
for (const entry of catalog.packs) {
  ...
  for (const [id, u] of Object.entries(p.units)) {
    try {
      ...
      throw new Error(`Runtime validation failed ${entry.id}/${id}: ${cause.message}`, {
        cause,
      });
    }
    assert(
    ...
    for (const s of u.motorStrokes) {
      ...
      assert(
        points.every((pair) => pair.length === 2 && pair.every(Number.isFinite)),
        'Invalid point',
      );
...
assert(
  sha256(await readFile(path.join(sourceDir, lock.mapping.file))) === lock.mapping.sha256,
...
for (const entry of catalog.rawObservations) {
  ...
  const data = JSON.parse(gunzipSync(bytes, { maxOutputLength: 64 * 1024 * 1024 }));
