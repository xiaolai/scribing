...
# Encoding and newline are explicit: the defaults are locale and platform dependent,
# and every byte here is hashed into fonts/motor/index.json and checked against it.
def write(p,d):p.write_text(json.dumps(d,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8',newline='\n')
def need(condition,message):
 if not condition:raise SystemExit('build-motors: %s'%(message,))
notices=out/'notices';notices.mkdir(exist_ok=True)
# Only the notices this run copied. Indexing the directory instead pulled in whatever
# else was sitting there, including a .DS_Store, and hashed it into the manifest.
copied_notices=[]
records=[]
for group,names in [('english',['english-textbook']),('korean',['korean-textbook']),('japanese',sorted(p.stem.replace('.manifest','') for p in (ROOT/'packs/generated').glob('japanese-*.manifest.json')))]:
 ...
 for name in names:
  ...
  if group!='japanese' or name=='japanese-kana':
   target=notices/(name+'.NOTICE.txt');shutil.copyfile(notice,target);copied_notices.append(target)
...
source=ROOT/'node_modules/hanzi-writer-data';need(read(source/'package.json')['version']=='2.0.1','hanzi-writer-data must be 2.0.1')
units={}
...
file=out/'chinese.json';write(file,{'schemaVersion':1,'units':units});arphic=notices/'ARPHICPL.TXT';shutil.copyfile(source/'ARPHICPL.TXT',arphic);copied_notices.append(arphic)
